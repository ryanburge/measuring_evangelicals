...
cces16 %>%  filter(pew_bornagain ==1 & race ==1 & complete.cases(CC16_410a)) %>% 
  count(CC16_410a, wt = commonweight_vv) %>% 
...
cces16 %>%  filter(pew_bornagain ==1 & race ==1 & religpew ==1 & complete.cases(CC16_410a)) %>% 
  count(CC16_410a, wt = commonweight_vv) %>% 
  mutate(weight = prop.table(n)) %>% arrange(desc(weight))
