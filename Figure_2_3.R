...
mean4 <-  cces2008 %>%  filter(evangelical ==1 & V211 ==1 & complete.cases(pid7)) %>% 
   summarise(mean = mean(pid7)) %>% 
   mutate(type = c("Affiliation"), year = c("CCES 2008")) 

mean5 <-  cces12 %>%  filter(evangelical ==1 & white ==1 & complete.cases(pid7)) %>% 
   summarise(mean = mean(pid7)) %>% 
  mutate(type = c("Affiliation"), year = c("CCES 2012")) 
 
mean6 <-  cces16 %>%  filter(evangelical ==1 & white ==1 & complete.cases(pid7)) %>% 
   summarise(mean = mean(pid7)) %>% 
   mutate(type = c("Affiliation"), year = c("CCES 2016")) 
...
ggsave(file="pid_dot_plot.png", type = "cairo-png", width = 12, height = 8)
...
ggsave(file="attend_dot_plot.png", type = "cairo-png", width = 12, height = 8)
